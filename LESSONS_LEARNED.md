...
- **Solução**:
  1. **Se `i` não é usado**: remover do callback → `array.map((item) => { … })`. É a forma mais limpa.
  2. **Se `i` será necessário em breve mas não agora**: prefixar com `_` → `array.map((item, _i) => { … })`. O TS ignora parâmetros prefixados com `_`. Útil quando você já sabe que vai precisar depois, ou em handlers de assinatura fixa (ex.: callbacks de bibliotecas) onde o segundo param é obrigatório.
  3. **Nunca** fingir que está usando: `void i;` é gambiarra que polui o diff.
- **Regra geral / pré-push**: Antes de cada `git push`, rodar mentalmente um grep por `\.map\(\(.+,\s*\w+\)` e verificar se o segundo parâmetro é REALMENTE referenciado no corpo. Mesmo padrão vale para `.forEach`, `.filter`, `.reduce`, `.some`, `.every`. Em pull requests envolvendo refactor de `style={{ animationDelay: i * X }}` ou similares, este é o ponto mais frequente de regressão TS6133.
- **Contexto**: Ocorrido em `src/ui/landing/mocks/MockBatchMove.tsx:52` durante a entrega da Landing Page Visual Showcase. O `i` foi mantido depois que o `style={...}` que dependia dele migrou para keyframes pré-definidos no LandingPage.tsx. Build do Vercel falhou em `tsc -b && vite build`. Corrigido em commit subsequente removendo o parâmetro.
