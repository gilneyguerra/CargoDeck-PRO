...
- **DANFE extraction** uses a **two-tier strategy**: [`src/services/danfeExtractor.ts`](src/services/danfeExtractor.ts) is the orchestrator that tries [`src/services/danfeParser.ts`](src/services/danfeParser.ts) first (deterministic regex over the SEFAZ-regulated table layout, zero network, ~100ms). LLM (`DANFE_EXTRACTION` task) only fires as fallback when the parser returns 0 items — handles atypical PDFs that don't match the standard 15-column tail pattern. Result includes `strategy: 'parser' | 'llm'` so the UI can label the toast accurately. Lesson #18 documents why structural parsing is preferred over LLM for regulated documents.
